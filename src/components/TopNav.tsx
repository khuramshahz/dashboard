...
export default function TopNav({ onToggleSidebar, onShowSystemLogs, onShowSecurityAudit }: TopNavProps) {
        ...
        <>
          ...
          <div className="fixed top-16 left-2 right-2 w-auto sm:left-auto sm:right-6 sm:w-80 bg-gradient-to-br from-[#0d1f3a] to-[#0A192F] border border-[#0096FF]/20 rounded-lg shadow-2xl shadow-black/50 z-[60]">
            ...
            <div className="p-2 space-y-1 max-h-[60vh] overflow-y-auto">
              {[
                { label: 'User Profile', icon: User, desc: 'Lead Forensic Analyst' },
                { label: 'System Preferences', icon: Activity, desc: 'Display & UI Config' },
                { label: 'Security Protocols', icon: Lock, desc: 'Access & Encryption' },
                { label: 'Database Config', icon: Database, desc: 'Storage & Sync' },
                ...
                <button
                  key={idx}
                  onClick={item.action}
                  className="w-full flex items-center gap-2 sm:gap-3 p-2 sm:p-3 hover:bg-[#0096FF]/10 rounded-lg transition-all group text-left"
                  type="button"
                >
